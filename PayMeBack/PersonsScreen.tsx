...
function PersonScreen() {
  ...
  const updateTransactionsName = async (newName: string) => {
  ...
  const removePerson = async () => {
    try {
      ...
      if (value !== null) {
        ...
        all_data.contacts = all_data.contacts.filter(
          (contact: {name: string}) => contact.name !== name,
        );
        all_data.transactions = all_data.transactions.filter(
          (transaction: {person: string}) => transaction.person !== name,
        );
  ...
  const getContactBalance = (name: string) => {
    let totalAmount = 0;
  ...
  const updateContactsBalances = async (data: any[]) => {
    try {
  ...
  const getCurrency = async () => {
    try {
      ...
      // error reading value
      console.error("could not get currency: ", e);
    }
  };
  ...
  const getData = async () => {
    try {
      ...
      if (value !== null) {
        ...
        setContacts(new_data as Contact[]); // Add type assertion here
      } else {
        //value not found in storage, store sample_transactions
        const sample_data = {
          transactions: [
          ...
          contacts: [
            {name: 'Derick', amount: 'R200', id: '#8923912', contactID: '#123'},
          ],
        };
      ...
      console.error("Error with get data: ", e);
    }
  };
                ...
                <List.Item
                  ...
                  description={
                    Number(contact.amount) < 0
                      ? `-${Currency}${Math.abs(Number(contact.amount))}`
          ...
          <TouchableWithoutFeedback onPress={() => setModalVisible(false)}>
            <View
              ...
              <View
                ...
                <Text style={{color: colors.text, fontSize: 20}}>
                  Type below the new name you'd like for this contact. All
            ...
            <View
              ...
              <View
                ...
                <Text
                  ...
                  Mark contact as paid off?
                </Text>
                <Text style={{color: colors.text, fontSize: 20}}>
                  This will remove the person and all their transactions. Are
                  you sure you'd like to mark this as paid off?
                </Text>
